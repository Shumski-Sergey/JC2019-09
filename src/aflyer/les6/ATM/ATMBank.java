...
public class ATMBank {
    ...
    public void GivinMoney() {
        ...
        boolean p = true;
        if ((hndrt == 0) && (ffty == 0) && (twnty == 0)) {
            System.out.println("В банкомате нету дзенех!");
            p = false;
        }
        if (sum > getAllCash()) {
            System.out.println("Сумма превышает количество купюр");
            p = false;
        }
            ...
            for (int j = 0; j <= ffty; j++) {
                for (int k = 0; k <= twnty; k++) {
                    if (i * HUNDRET + j * FIFTY + k * TWENTY == sum) {
                        if ((i != 0) && (j != 0) && (k != 0))
                            System.out.println("Можем выдать " + i + " по " + HUNDRET + " + " + j + " по " + FIFTY + " + " + k + " по " + TWENTY);
                        if ((i == 0) && (j != 0) && (k != 0))
                            System.out.println("Можем выдать " + j + " по " + FIFTY + " + " + k + " по " + TWENTY);
                        if ((i != 0) && (j == 0) && (k != 0))
                            System.out.println("Можем выдать " + i + " по " + HUNDRET + " + " + k + " по " + TWENTY);
                        if ((i == 0) && (j != 0) && (k == 0))
                            System.out.println("Можем выдать " + i + " по " + HUNDRET + " + " + j + " по " + FIFTY);
                        if ((i == 0) && (j == 0) && (k != 0)) System.out.println("Можем выдать " + k + " по " + TWENTY);
                        if ((i == 0) && (j != 0) && (k == 0)) System.out.println("Можем выдать " + j + " по " + FIFTY);
                        if ((i != 0) && (j == 0) && (k == 0))
                            System.out.println("Можем выдать " + i + " по " + HUNDRET);
                        q += 1;
                    }
                }
            }
